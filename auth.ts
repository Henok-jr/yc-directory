...
// Server-side session helper (used by server components/actions)
export async function auth() {
  return getServerSession(authOptions);
}

// Server Actions (used by server components). These wrappers keep your existing imports working.
export async function signIn(provider?: string, options?: any) {
  // In v4, programmatic server-side signIn is not supported the same way as v5.
  // Prefer client-side `signIn` or posting to /api/auth/signin.
